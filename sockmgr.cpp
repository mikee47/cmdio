...
class CWebsocketResource: public WebsocketResource
{
  protected:
    // virtual
  ...
  public:
    CWebsocketResource() : WebsocketResource()
    {
      onHeadersComplete = HttpResourceDelegate(&CWebsocketResource::checkHeaders, this);
      onUpgrade = HttpServerConnectionUpgradeDelegate(&CWebsocketResource::processData, this);
    }
 ...
 */
void CSocketManager::loginComplete(command_connection_t connection, JsonObject& json)
{
  /*
  ...
  for (unsigned i = 0; i < m_handlers.count(); ++i) {
    CCommandHandler* handler = m_handlers[i];
...
CCommandHandler* CSocketManager::findHandler(const char* method)
{
  for (unsigned i = 0; i < m_handlers.count(); ++i) {
    CCommandHandler* handler = m_handlers[i];
...
void CSocketManager::connected(WebSocketConnection& socket)
{
  auto cc = CWSCommandConnection::fromSocket(&socket);
...
void CSocketManager::disconnected(WebSocketConnection& socket)
{
  auto cc = CWSCommandConnection::fromSocket(&socket);
 ...
 */
void CSocketManager::messageReceived(WebSocketConnection& socket, const String& message)
{
  const char* MSG_PING = "?";
  const char* MSG_PONG = "#";
  if (message == MSG_PING) {
 ...
 *
 */
void CSocketManager::binaryReceived(WebSocketConnection& socket, uint8_t* data, size_t size)
{
  auto cc = CWSCommandConnection::fromSocket(&socket);
...
WebsocketResource* CSocketManager::createResource()
{
  // Web Sockets configuration
  auto wsResource = new CWebsocketResource();
  wsResource->setConnectionHandler(connected);
  wsResource->setDisconnectionHandler(disconnected);
  wsResource->setMessageHandler(WebSocketMessageDelegate(&CSocketManager::messageReceived, this));
